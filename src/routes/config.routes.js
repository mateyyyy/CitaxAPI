...
router.put('/company-profile', requireRole('admin_empresa'), async (req, res) => {
    try {
        ...
        if (normalizedSlug.length < 3) {
            return res.status(400).json({ error: 'El subdominio debe tener al menos 3 caracteres' });
        }
        ...
        if (existingRows.length > 0) {
            return res.status(409).json({ error: 'Ese subdominio ya esta en uso por otra cuenta' });
        }

        await pool.execute(
            'UPDATE EMPRESA SET slug = ? WHERE id_empresa = ?',
            [normalizedSlug, req.user.id_empresa]
        );
    ...
    try {
        ...
        if (req.user.rol === 'admin_empresa') {
            ...
            if (duplicatedCompanyName) {
                await connection.rollback();
                return res.status(409).json({ error: 'Ya existe otra empresa con ese nombre comercial' });
            }
        }
...
router.put('/bot', async (req, res) => {
    ...
    try {
        ...
        if (!validation.valid) {
            await connection.rollback();
            return res.status(400).json({ error: validation.reason || "Configuración rechazada por el sistema de seguridad (insegura)." });
        }
